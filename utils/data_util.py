"""
This file is used to read data and preprocess it. 
A function of using only part of the training data has been added, other parts are the same as the original code.
"""
...
class ReadData:
    def __init__(self, opt):
        ...
        # change: add function using only part of the training data
        if hasattr(opt, 'train_proportion') and opt.train_proportion < 1.0:
            self.train_examples, self.remaining_examples = self._subsample_training_data(
                self.train_examples, opt.train_proportion, opt.data_dir)
        else:
    ...
    def _subsample_training_data(self, train_examples, proportion, data_dir):
        """
        Subsample the training data and save the remaining data for training the XGBoost model
        """
        if proportion == 1.0:
        ...
        if hasattr(self.opt, 'seed'):
            FIXED_SEED = self.opt.seed       
        random.seed(FIXED_SEED)
        np.random.seed(FIXED_SEED)

        total_samples = len(train_examples)
        keep_samples = int(total_samples * proportion)
        selected_indices = set(random.sample(range(total_samples), keep_samples))
        subsampled_examples = []
        remaining_examples = []
        
        for i, example in enumerate(train_examples):
            if i in selected_indices:
                subsampled_examples.append(example)
            else:
                remaining_examples.append(example)
        
        # save remaining data for training the XGBoost model
        remaining_proportion = 1.0 - proportion
        remaining_dir = data_dir + f"_remaining_{remaining_proportion:.1f}"
        os.makedirs(remaining_dir, exist_ok=True)
        
        remaining_train_file = os.path.join(remaining_dir, "train.tsv")
        
        with open(remaining_train_file, 'w', encoding='utf-8') as f:
            for example in remaining_examples:
                aspect = example.aspect
                
                if hasattr(example, 'text_left') and example.text_left is not None:
                    full_text = example.text_a
                    left_part = example.text_left.strip()
                    
                    temp_text = full_text
                    if left_part:
                        temp_text = temp_text.replace(left_part, "", 1).strip()
                    temp_text = temp_text.replace(aspect, "", 1).strip()
                    right_part = temp_text
                    
                    if left_part and right_part:
                        text_with_marker = f"{left_part} $T$ {right_part}"
                    elif left_part:
                        text_with_marker = f"{left_part} $T$"
                    elif right_part:
                        text_with_marker = f"$T$ {right_part}"
                    else:
                        text_with_marker = "$T$"
                else:
                    text_with_marker = example.text_a.replace(aspect, "$T$", 1)
                
                text_with_marker = " ".join(text_with_marker.split())
                
                f.write(f"{text_with_marker}\n")
                f.write(f"{aspect}\n") 
                f.write(f"{example.label}\n")
        
        original_dev_file = os.path.join(data_dir, "dev.tsv")
        if os.path.exists(original_dev_file):
            remaining_dev_file = os.path.join(remaining_dir, "dev.tsv")
            import shutil
            shutil.copy2(original_dev_file, remaining_dev_file)
        
        return subsampled_examples, remaining_examples
