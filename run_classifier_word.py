# coding=utf-8
"""
The training and validation program for the original TD-BERT project. 
It remains almost unchanged, with some modifications to the model loading method to accommodate PyTorch 2.6+ on the experimental equipment. 
The Chinese comments in the original project have been translated into English.
"""
...
class Instructor:
    def __init__(self, args):
        ...
        for p in self.model.parameters():
            n_params = torch.prod(torch.tensor(p.shape))  # torch.prod() calculates the product of all elements
            if p.requires_grad:  # Whether gradient calculation is needed
        ...
        self.global_step = 0  # Initialize global step to 0
        self.max_test_acc = 0
        self.max_test_f1 = 0

    def do_train(self):  # Train model
        for i_epoch in range(int(args.num_train_epochs)):
            ...
            y_pred = []
            y_true = []
            self.model.train()  # Set model to training mode, as it will be in test mode after each epoch
            ...
            result = self.do_eval()  # Test once after each epoch
            tr_loss = tr_loss / nb_tr_steps
            # self.scheduler.step(result['eval_accuracy'])  # Monitor validation set accuracy
    ...
    def do_eval(self):  # Test accuracy
        self.model.eval()
        ...
        if eval_accuracy > self.max_test_acc:
            ...
            if self.opt.do_predict:  # Save model only in test mode
    ...
    def do_predict(self):
        # Load saved model for prediction to get accuracy
        # Read test set data
        # dataset = ReadData(self.opt)  # This method is a bit redundant, reading all data including training set
        # Load model
